    ...
    <div class="card-body">
        ...
        <div class="mb-3">
            ...
            <span class="text-danger notice d-block mb-2 font-italic text-sm">* {{ __('messages.parentNotice') }}</span>
                <select name="post_catalogue_id" class="form-control form-control-sm  select2">  
                    @foreach($dropdown as $key => $val)
                    <option 
                        {{$key == old('post_catalogue_id', (isset($post->post_catalogue_id))? $post->post_catalogue_id : '') ? 'selected' : ''}} value="{{$key}}" >{{$val}}
                    </option>
                    @endforeach
                </select>
        </div>
...
@include('backend.layout.component.publish', ['model' => ($post) ?? null])
